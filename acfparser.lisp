        ...
        (cond ((string-equal arg1 "boost")
              ...
              ((string-equal arg1 "overdub")
               (if (not (cmd-keys arg2))
                   (mv (overdub (mv-let (wav3 state) (acf-function-h (cdr commands) wav state) wav3)
                                (mv-let (wav2 state) (read-wav arg2 state) wav2)) state)
                   (mv (overdub (mv-let (wav6 state) (acf-function-h (cdr commands) wav state) wav6)
                                (mv-let (wav4 state) (acf-function-h (list (butlast (cdr args) 1))
                                (mv-let (wav5 state) (read-wav (get-last args) state) wav5) state) wav4)) state)))
